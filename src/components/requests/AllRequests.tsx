...
export default function AllRequests() {
  ...
  } = useGetSentRequestsQuery(selectedType === "SENT" ? undefined : skipToken);
  return (
    <StyledMain>
      ...
      <StyledMainContainer>
        {selectedType !== "SENT" &&
          (isLoading ? (
            <StyledErrorMessage>Loading Requests...</StyledErrorMessage>
          ) : error ? (
            <StyledErrorMessage>Failed Loading Requests!</StyledErrorMessage>
          ) : data ? (
            (data.user.receivedRequest.length > 0 &&
              data.user.receivedRequest.map((request) => {
        ...
        {selectedType === "SENT" &&
          (sentLoading ? (
            <StyledErrorMessage>Loading Sent Requests...</StyledErrorMessage>
          ) : sentError ? (
            <StyledErrorMessage>
              Failed Loading Sent Requests!
            </StyledErrorMessage>
          ) : sentData ? (
            (sentData.user.sentRequest.length > 0 &&
              sentData.user.sentRequest.map((request) => {
                return <UserRequest key={request.id} info={request} />;
              })) || (
