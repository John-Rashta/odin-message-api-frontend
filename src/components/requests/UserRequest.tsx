      ...
      <>
        {info.sender.id === myId ? (
          <StyledButtonContainer>
            {" "}
            <StyledOptions onClick={() => deleteRequest({ id: info.id })}>
              Cancel
            </StyledOptions>{" "}
          </StyledButtonContainer>
        ) : (
          <StyledButtonContainer>
            <StyledAccept onClick={() => acceptRequest({ id: info.id })}>
              Accept
            </StyledAccept>
            <StyledOptions onClick={() => deleteRequest({ id: info.id })}>
              Reject
      ...
      <StyledReturn>
        <div>
          <div data-id={info.id} data-group={info.group.id}>
            {info.sender.id === myId
              ? `You invited ${info.receiver.username} to ${info.group.name}`
              : `${info.sender.username} invited you to ${info.group.name}`}
          </div>
          <div>
            {formatRelative(new Date(info.sentAt), new Date(), { locale })}
          </div>
        </div>
        {generateOptions()}
      </StyledReturn>
    ...
    return (
      <StyledReturn>
        <StyledText>
          <div data-id={info.id}>
            {info.sender.id === myId
              ? `You sent a Friend Request to ${info.receiver.username}`
