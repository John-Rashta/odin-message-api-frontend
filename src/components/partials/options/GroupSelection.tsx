...
export default function GroupSelection({
  ...
  groupsData: BasicGroupInfo[] | undefined;
  userid: string | undefined;
  hideMe: SimpleFunctionType;
}) {
  ...
  const handleClick = function handleClickingAnOption(event: ClickType) {
    ...
    if (possibleType === "INVITEGROUP") {
      ...
      makeRequest({ targetid: userid, type: "GROUP", groupid: possibleGroup })
        .unwrap()
        .then(() => {
          hideMe();
        })
  ...
  return <GroupOptions groupsData={groupsData} handleClick={handleClick} />;
}
