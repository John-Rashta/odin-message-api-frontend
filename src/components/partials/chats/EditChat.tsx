...
export default function EditChat({
  ...
  info: string;
  changeEdit: SimpleFunctionType;
}) {
  const [updateMessage] = useUpdateMessageMutation();
  const [textValue, setTextValue] = useState("");
  const [showEmotes, setShowEmotes] = useState(false);
  const { messageData } = useGetMessageQuery(
    { id: info },
    {
  ...
  const handleSubmit = function handleSubmitingEdit(event: FormType) {
    ...
    updateMessage({ message: { content: content }, id: messageData.id })
      .unwrap()
      .then()
      .catch((e) => {
        if (e.data.message) {
    ...
    <>
      {messageData && (
        ...
        >
          ...
          <StyledCancel onClick={() => changeEdit()} type="button">
            Cancel
          </StyledCancel>
