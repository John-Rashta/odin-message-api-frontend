...
export default function MessagesContainer({
  ...
  return (
    <StyledMessagesContainer
      ...
      onMouseOver={(e) => {
        ...
        handleMessageOptionsClick(
          myId,
          e,
          getFuncs([showFuncs, coordsFuncs, dataFuncs]),
        );
      }}
      onMouseOut={(e) => {
    ...
    >
      {showFuncs.checkShow && dataFuncs.checkData.message ? (
        <MessageOptions
          checkId={checkId}
          changeId={setEditId}
          coords={coordsFuncs.checkCoords}
          changeVisible={() => showFuncs.changeShow(false)}
          messageid={dataFuncs.checkData.message}
        />
      ) : null}
      {info.map((currentMessage, index) => {
        return (
          <Message
            ...
            info={currentMessage}
            {...(index > 0
              ? { previousMessageSenderId: info[index - 1].sender.id }
              : {})}
