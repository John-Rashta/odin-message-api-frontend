...
export default function Message({
  ...
  return (
    <StyledMessage
      data-messageid={info.id}
      data-userid={info.sender.id}
      className="optionsMessage"
    >
      <StyledIconContainer>
        {info.sender.id === previousMessageSenderId ? null : (
          <StyledIcon
            ...
            data-userid={info.sender.id}
            {...(adminList && adminList.includes(info.sender.id)
              ? { "data-admin": true }
              : {})}
            src={
              info.sender.customIcon
                ? info.sender.customIcon.url
                : info.sender.icon.source
            }
            alt="user icon"
          />
        )}
