    ...
    return (
        ...
        onClick={(e) =>{
            ...
            if (full.showFuncs.checkShow) {
                ...
                {full.showFuncs.checkShow && <UserOptions info={full.dataFuncs.checkData} changeVisible={() => full.showFuncs.changeShow(false)} coords={full.coordsFuncs.checkCoords} {...((typeof group === "string" && members && members.includes(full.dataFuncs.checkData.user || "joe")) ? {group} : {})} />}
                {min.showFuncs.checkShow && <MinimalUserOptions info={min.dataFuncs.checkData} changeVisible={() => min.showFuncs.changeShow(false)} coords={min.coordsFuncs.checkCoords} {...((typeof group === "string" && members && members.includes(full.dataFuncs.checkData.user || "joe")) ? {group} : {})} />}
            {children}
        </main>
    )
}