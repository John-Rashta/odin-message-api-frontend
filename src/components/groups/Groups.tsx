...
export default function Groups() {
  ...
  const handleClick = function handleClickingToLeaveGroup(
  ...
  const handleSubmitInvite = function handleSubmitingAnInvite(
    event: FormType,
    fieldName: string,
  ) {
    event.preventDefault();
  ...
  return (
    <StyledMain
      {...(data && userIsAdmin ? { group: groupId, members: membersIds } : {})}
    >
      <MainStyleDivs>
        ...
        {groupsLoading ? (
          <StyledSideError>Groups Loading...</StyledSideError>
        ) : groupsError ? (
          <StyledSideError>Error Loading Groups!</StyledSideError>
        ) : groupsData && groupsData.user.groups.length > 0 ? (
          <SideBar
            data={groupsData.user.groups}
            activeId={groupId}
            innerComp={MiniGroupSide}
          />
        ) : (
      ...
      {isLoading ? (
        <StyledExtraMessage>Loading Group...</StyledExtraMessage>
      ) : data && !error ? (
        <>
          <MainStyleDivs>
            <StyledGroupOptions>
              {userIsAdmin && (
                <>
                  ...
                  />
                  <StyledForm
                    submitFunc={(e) => handleSubmitInvite(e, "inviteUser")}
                    fieldsName="inviteUser"
                    buttonText="Invite"
                  />
                </>
              )}
              <StyledGroupOptionsBottom>
                ...
                {userIsAdmin &&
                  (showConfirm ? (
                    <StyledDeleteOptions>
                      {" "}
                      <StyledDeleteCancel onClick={() => setShowConfirm(false)}>
                        Cancel
                      </StyledDeleteCancel>{" "}
                      <StyledDeleteConfirm
                        onClick={() => {
                    ...
                    <StyledDelete onClick={() => setShowConfirm(true)}>
                      Delete Group
                    </StyledDelete>
                  ))}
              </StyledGroupOptionsBottom>
            ...
            <FullChat
              adminList={getAdminIds(data.group.admins)}
              basicInfo={{ id: data.group.id, type: "GROUP" }}
              trigger={sendMessage}
              info={data.group.contents}
            />
          </MainStyleDivs>
          <GroupPeople
