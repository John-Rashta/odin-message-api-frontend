...
export default function EditGroupName({
  ...
  const handleSubmit = function handleSubmitingNameChange(
    event: FormType,
    fieldName: string,
  ) {
    ...
    updateName({ id: groupId, info: { name: possibleName } })
      .unwrap()
      .finally(() => {
        setEditName(false);
      });
      ...
      {editName ? (
        <StyledEdit>
          <StyledForm
            ...
            possibleDefault={groupName || ""}
            buttonText={buttonText}
            submitFunc={(e) => handleSubmit(e, fieldsName)}
          />
          <StyledCancel onClick={() => setEditName(false)}>Cancel</StyledCancel>
        </StyledEdit>
      ) : (
        <StyledButton onClick={() => setEditName(true)}>Edit Name</StyledButton>
      )}
    </div>
  );
}
