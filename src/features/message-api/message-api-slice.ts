...
export const apiSlice = createApi({
  ...
  baseQuery: fetchBaseQuery({
    baseUrl: `https://${String(process.env.VITE_API_URL)}`,
    credentials: "include",
  }),
  tagTypes: [
    "SelfInfo",
        ...
        const response = {
          group: {
            ...
            members: responseData.group.members.map((member) => {
              return {
                ...member,
                admin: responseData.group.admins.some(
                  (admin) => member.id === admin.id,
                ),
              };
            }),
          },
