...
export default function useClickOutside(
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        currentTarget.closest(".optionsUser") &&
        ref.current?.classList.contains("userOptions")
      ) {
        return;
      }

      if (
        currentTarget.closest(".otherOptions") &&
        ref.current?.classList.contains("extraOptions")
      ) {
        return;
      }
